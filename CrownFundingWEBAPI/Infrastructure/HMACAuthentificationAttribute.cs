...
using CrownFundingWEBAPI.Controllers;
using CrownFundingWEBAPI.Models;

namespace CrownFundingWEBAPI.Infrastructure
{
    public class HMACAuthentificationAttribute : Attribute, IAuthenticationFilter
    {
            private static Dictionary<string, string> allowedApps = new Dictionary<string, string>();
            private readonly UInt64 requestMaxAgeInSeconds = 300;  //5 mins
            private readonly string authenticationScheme = "amx";

            //public HMACAuthentificationAttribute()
            //{
            //    if (allowedApps.Count == 0)
            //    {
            //        Encryptor ec = new Encryptor();
            //        UtilisateurController us = new UtilisateurController();
            //        IEnumerable<VUtilisateur> all = us.GetAll();
            //        if (all != null)
            //        {
            //            foreach (var item in all.Where(u => u.IsActif))
            //            {
            //                allowedApps.Add(item.Email, ec.ByteToString(item.SecretKey));
            //            }
            //        }
            //    }
            //}
